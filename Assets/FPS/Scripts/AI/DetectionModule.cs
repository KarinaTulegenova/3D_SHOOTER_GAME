...
namespace Unity.FPS.AI
{
    public class DetectionModule : MonoBehaviour
    {
        ...
        {
            if (m_ActorsManager == null || actor == null)
                return;

            Transform detectionSource = DetectionSourcePoint != null ? DetectionSourcePoint : transform;
            ...
            {
                if (otherActor == null || otherActor.AimPoint == null)
                    continue;

                if (otherActor.Affiliation != actor.Affiliation)
                {
                    float sqrDistance = (otherActor.transform.position - detectionSource.position).sqrMagnitude;
                    if (sqrDistance < sqrDetectionRange && sqrDistance < closestSqrDistance)
                    {
                        // Check for obstructions
                        RaycastHit[] hits = Physics.RaycastAll(detectionSource.position,
                            (otherActor.AimPoint.position - detectionSource.position).normalized, DetectionRange,
