...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            RepairFirstPersonWeaponHierarchy();
            SetFov(DefaultFov);
        ...
        void Update()
        {
            ...
            if (activeWeapon)
            {
                Camera aimCamera = WeaponCamera != null ? WeaponCamera : m_PlayerCharacterController != null ? m_PlayerCharacterController.PlayerCamera : null;
                if (aimCamera != null && Physics.Raycast(aimCamera.transform.position, aimCamera.transform.forward, out RaycastHit hit,
        ...
        public bool AddWeapon(WeaponController weaponPrefab)
        {
            ...
            if (WeaponParentSocket == null)
            {
                Debug.LogError("Cannot add first-person weapon because WeaponParentSocket is not assigned.", this);
                return false;
            }
            ...
            for (int i = 0; i < m_WeaponSlots.Length; i++)
            {
                ...
                if (m_WeaponSlots[i] == null)
                {
                    ...
                    weaponInstance.name = weaponPrefab.name + "_FP";
        ...
        void RepairFirstPersonWeaponHierarchy()
        {
            if (WeaponParentSocket == null)
            {
                Debug.LogError("WeaponParentSocket is not assigned. First-person weapons need their own socket under the first-person camera.", this);
                return;
            }

            if (m_PlayerCharacterController == null || m_PlayerCharacterController.PlayerCamera == null)
            {
                Debug.LogError("PlayerCamera is not assigned. First-person weapon aiming and socket repair require the main first-person camera.", this);
                return;
            }

            Transform firstPersonSocket = WeaponParentSocket.parent;
            if (firstPersonSocket == null)
                return;

            bool socketLooksLikeFirstPersonSocket = firstPersonSocket.name.Contains("FirstPersonSocket");
            bool socketUnderFirstPersonCamera = firstPersonSocket.IsChildOf(m_PlayerCharacterController.PlayerCamera.transform);

            if (socketLooksLikeFirstPersonSocket && !socketUnderFirstPersonCamera)
            {
                Debug.LogWarning("FirstPersonSocket was not under the first-person camera. Reparenting it to PlayerCamera at runtime.", this);
                firstPersonSocket.SetParent(m_PlayerCharacterController.PlayerCamera.transform, false);
                firstPersonSocket.localPosition = Vector3.zero;
                firstPersonSocket.localRotation = Quaternion.identity;
                firstPersonSocket.localScale = Vector3.one;
            }
        }
    }
}
