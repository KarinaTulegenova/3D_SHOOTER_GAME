...
namespace Unity.FPS.Gameplay
{
    public class ProjectileStandard : ProjectileBase
    {
        ...
        [Tooltip("Small fallback radius used to catch enemies with simple capsule hitboxes")]
        public float FallbackDamageRadius = 0.6f;
        ...
        new void OnShoot()
        {
            ...
            PlayerWeaponsManager playerWeaponsManager = m_ProjectileBase.Owner != null
                ...
                : null;
            if (playerWeaponsManager != null && playerWeaponsManager.enabled && playerWeaponsManager.WeaponCamera != null)
            {
        ...
        bool IsHitValid(RaycastHit hit)
        {
            ...
            // Ignore triggers that are not part of a damage receiver (volumes without Health/Damageable).
            // Allow triggers that have Health so third-person weapon/hurt volumes still register hits.
            if (hit.collider.isTrigger)
            {
                if (hit.collider.GetComponentInParent<Damageable>() != null)
                    return true;
        ...
        void TryDamageNearbyHealth(Vector3 point)
        {
            if (FallbackDamageRadius <= 0f)
                return;

            Collider[] colliders = Physics.OverlapSphere(point, FallbackDamageRadius, HittableLayers, k_TriggerInteraction);
            foreach (Collider nearbyCollider in colliders)
            {
                Health health = nearbyCollider.GetComponentInParent<Health>();
                if (health != null && health.gameObject != m_ProjectileBase.Owner)
                {
                    health.TakeDamage(Damage, m_ProjectileBase.Owner);
                    return;
                }
