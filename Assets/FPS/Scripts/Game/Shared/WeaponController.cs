...
namespace Unity.FPS.Game
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        void HandleShoot()
        {
            ...
            if (WeaponAnimator && HasAnimatorParameter(WeaponAnimator, k_AnimAttackParameter, AnimatorControllerParameterType.Trigger))
            {
        ...
        public Vector3 GetShotDirectionWithinSpread(Transform shootTransform)
        {
            ...
            Actor ownerActor = Owner != null ? Owner.GetComponent<Actor>() : null;
            bool ownerIsPlayer = Owner != null &&
                (Owner.CompareTag("Player") || (ownerActor != null && ownerActor.Affiliation == 1));

            if (ownerIsPlayer && Camera.main != null)
            {
                baseDirection = Camera.main.transform.forward;
            }
            else if (Owner == null && Camera.main != null)
            {
                baseDirection = Camera.main.transform.forward;
            }

            float spreadAngleRatio = BulletSpreadAngle / 180f;
            Vector3 spreadWorldDirection = Vector3.Slerp(baseDirection, UnityEngine.Random.insideUnitSphere,
                spreadAngleRatio);

            return spreadWorldDirection;
        }

        bool HasAnimatorParameter(Animator animator, string parameterName, AnimatorControllerParameterType parameterType)
        {
